...
* [6.1 Requirements](#software)
* [6.2 Test the effect of different clock models (strict vs. relaxed clock)](#clock_models)
	* [6.2.1 Infer the Charadriidae phylogeny using the strict clock model](#co1_strict)
		* [6.2.1.1 Generate a XML configuration file in BEAUti](#beauti)
		* [6.2.1.2 Inspect the XML configuration file](#xml)
		* [6.2.1.3 Run the MCMC analysis using BEAST 2](#beast)
		* [6.2.1.4 Visually inspect the posterior estimates using Tracer](#tracer)
	* [6.2.2 Infer the Charadriidae phylogeny using the relaxed clock model](#co1_relaxed)
	* [6.2.3 Compare the results of the strict and relaxed clock analyses](#strict_vs_relaxed)
		* [6.2.3.1 Compare the log files using Tracer](#compare_tracer)
		* [6.2.3.2 Compare the tree files using TreeAnnotator and FigTree](#compare_trees)
* [Optional: Sample from the prior distribution and ignore the data](#nodata)
* [6.3 Estimate the time of arrival of the endemic New Zealand shorebirds](#dispersal_dating)
...
<details>
 <summary>Background, software, and data (click here)</summary>
...
## 6.2 Test the effect of different clock models (strict vs. relaxed clock)
...
<a name="co1_strict"></a>
### 6.2.1 Infer the Charadriidae phylogeny using the strict clock model

<a name="beauti"></a>
#### 6.2.1.1 Generate a XML configuration file in BEAUti

Download the [`co1.nexus`](data/co1.nexus). Open the program BEAUti (“Bayesian Evolutionary Analysis Utility”), and click “**Import Alignment**” in BEAUti’s “**File**” menu. Select the co1 alignment file that you just downloaded (if you get the error "unsupported sequence file", close BEAUti, re-open, and try again.
...
Click on the **triangle** next to "**BDDeathRate**" so see the prior contains. The relative extinction rate is typically constrained to lie between 0 and 1, because if it was greater than 1, this would mean that the extinction rate is greater than the speciation rate, and in such a case, clades would go extinct rather than diversify.
...
This clades should not be constrained with an age but instead to be monophyletic, so check the “**monophyletic**” box on the very right of this prior. 
...
Finally, click on the last tab named “**MCMC**”. Here, set the “**Chain Length**” to 5,000,000 steps of the MCMC chain, and leave the other fields for “Store Every”, “Pre Burnin”, and “Num Initialization Attempts” as they are. 

<kbd>![](./img/beauti_009.png)</kbd>

Click on the triangle to the left of “**tracelog**”, where you can specify the name of the log file written by BEAST 2. Specify `co1_strict_clock.log`.
...
<a name="xml"></a>
#### 6.2.1.2 Inspect the XML configuration file
...
<a name="beast"></a>
#### 6.2.1.3 Run the MCMC analysis using BEAST 2

Open the software BEAST 2 and click on “**Choose File...**” to select the XML input file [`co1_strict_clock.xml`](res/co1_strict_clock.xml). You may choose more than 1 thread depending on your computer, but just leave everything else at their default values and click “**Run**” to start the analysis. This should take no more than 5 minutes.  
Note: Check how many minutes/Msamples are needed – sometimes, BEAST 2 runs faster on one or two threads than when using more.
...
<a name="tracer"></a>
#### 6.2.1.4 Visually inspect the posterior estimates using Tracer
...
**A graph of MCMC samples** is shown in the bottom right. By default these are shown in the form of a histogram, but when you click on the tab for “**Trace**” (at the very top right), you will see how the states of these samples have changed over the course of the MCMC. If this plot has the form of a “hairy caterpillar” that lies horizontally, the parameter estimates can be considered converged.

<kbd>![](./img/tracer_001.png)</kbd>

![](../img/question_icon.png) In the “**Traces**” panel, click on the parameter “**TreeHeight**”. This is the posterior estimate for the length of the tree from the root to the tips, which is equivalent to the age of the root. We set a prior on this age that specified the age to have a mean of 50 with a standard deviation of +/- 1. Have a look at the summary statistics, what are the values for mean and the 95% HPD (“highest posterior density”) interval? 
...
## 6.2.2 Infer the Charadriidae phylogeny using the relaxed clock model
...
--------
If the standard deviation would be zero, there would be no variation in rates among branches, meaning that the model would be similar to a strict clock. 
...
As before, use BEAST 2 to run an analysis, but now use the XML file [`co1_relaxed_clock.xml`](./res/co1_relaxed_clock.xml) as input. You may choose more than 1 thread depending on your computer, but just leave everything else at their default values and click “**Run**” to start the analysis. On only 1 CPU, this should take no more than 10 minutes.


<a name="strict_vs_relaxed"></a>
## 6.2.3. Compare the results of the strict and relaxed clock analyses

<a name="compare_tracer"></a>
### 6.2.3.1 Compare the log files using Tracer
...
**Note**: If you are tight on time, please proceed directly to [part 6.3](#dispersal_dating), our most important analysis where the dispersal time of the four endemic New Zealand species is estimated.
...
--------
Without data (left image), the posterior corresponds to the prior, with data (right image), our prior assumptions are updated from the data and thus the posterior distribution should differ from the prior distribution.
...
![](../img/question_icon.png) In the "**Trace File**" panel, select the `co1_relaxed_clock_from_prior.log` file. Then, click on the parameter “**BDDeathRate**”, the “relative extinction rate". We specified this rate to be between 0 and 1 using a uniform prior. In the graph of the "**Estimates**" tab, you can see that our MCMC has sampled the entire uniform distribution between 0 and 1 almost homogeneously. 
...
## 6.3 Estimate the time of arrival of the endemic New Zealand shorebirds
...
Continue to the “**MCMC**” tab. Use 5,000,000 as the chain length, but change the names of the output files again: Click on the triangle to the left of “**tracelog**” and specify `div_date.log`. Then click on the black triangle to the left of “**treelog**”. Specify `div_date.trees` as the file name. Finally, save the XML file as `div_date.xml`, open it in BEAST 2 and run the MCMC search. Adjust the CPU usage (check how many minutes/Msamples are needed) – sometimes BEAST 2 runs faster on one or two threads than on more. The analysis will take between 15 to 30 minutes, so take a break!
...
![](../img/question_icon.png) Open the file [`div_date.log`](./res/div_date.log) in Tracer and find out if the MCMC search has converged (*i.e.*, do all parameters have ESS values above 200 in the bottom left part of the Tracer window?
